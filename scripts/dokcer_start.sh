...
MongoDB
docker run -itd  -v /data/share:/data/share -p27017:27017  -v /data/app:/application -v  /etc/resolv.conf:/etc/resolv.conf  -v /etc/hosts:/etc/hosts -v /sys/fs/cgroup:/sys/fs/cgroup  --net myvpc  --ip 10.10.10.11  --cap-add=SYS_PTRACE --cap-add=NET_ADMIN  --privileged=true --name mongodb   -h mongodb  mongodb:latest  /usr/sbin/init
